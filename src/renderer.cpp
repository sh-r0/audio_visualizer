...
VkImageView createImageView(VkImage _image, VkFormat _format, VkDevice _device) {
...
uint32_t findMemoryType(uint32_t _typeFilter, VkMemoryPropertyFlags _properties, VkPhysicalDevice _physicalDevice) {
...
void renderer_t::createBuffer(VkDeviceSize _size, VkBufferUsageFlags _usage, VkMemoryPropertyFlags _properties, VkBuffer& _buffer, VkDeviceMemory& _bufferMemory) {
    ...
    allocInfo.memoryTypeIndex = findMemoryType(memRequirements.memoryTypeBits, _properties, physicalDevice);
...
void renderer_t::updateUBO() {
    uniformBuffer_t ubo{};
    for (uint32_t i = 0; i < barsCount / 4; i++) {
        ubo.heights[i].r = heights[i*4];
        ubo.heights[i].g = heights[i*4+1];
        ubo.heights[i].b = heights[i*4+2];
        ubo.heights[i].a = heights[i*4+3];
    }
...
void renderer_t::recordCommandBuffer(VkCommandBuffer _commandBuffer, uint32_t _imageIndex) {
    ...
    vkCmdDrawIndexed(_commandBuffer, static_cast<uint32_t>(indices.size()), barsCount, 0, 0, 0);
